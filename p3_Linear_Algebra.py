...
plt.arrow(0, 0, *v, color='b', linestyle='dotted', linewidth=2.0, head_length=0.5, head_width=0.4)
plt.arrow(0, 0, *vv, color='b', linewidth=2, head_length=0.5, head_width=0.4)
plt.arrow(0, 0, *w, color='g', linestyle='dotted', linewidth=2.0, head_length=0.5, head_width=0.4)
plt.arrow(0, 0, *nw, color='g', linestyle='dotted', linewidth=2.0, head_length=0.5, head_width=0.4)

plt.arrow(vv[0], vv[1], *nw, color='g', linewidth=1.2, head_length=0.5, head_width=0.4)

plt.arrow(0, 0, *y, linewidth=2.0, head_length=0.5, head_width=0.4)
...
vector_of_scalars = np.asarray([None] * 2)

try:
    vector_of_scalars = np.linalg.solve(set_of_vectors, vector_to_check)
    if not (vector_of_scalars is None):
        print('方程的解为：', vector_of_scalars)
except Exception as exception_type:
    print('异常', exception_type)

plt.plot(vector_of_scalars[0], vector_of_scalars[1], 'ro')
...
left = np.matmul(A, B_plus_C)

A_mul_B = np.matmul(A, B)
A_mul_C = np.matmul(A, C)

right = A_mul_B + A_mul_C
...
# 回答：$A^{-1}Ax = A^{-1}b$ => $Ix = A^{-1}b$ => $x = A^{-1}b$
# 
...
# 2.2.2 TODO
diag_mat = np.diag(range(3, 9))
mat_T = np.array([6, 7, 1, 2, 5, 9]).T
mul_m = np.matmul(diag_mat, mat_T)
mul_m
...
def calc_Norm(x, p = 2, infty = False):
    if not (str(type(x)) == "<class 'numpy.ndarray'>"):
        x = np.array(x)
    x = x.astype(float)
    x_abs = np.absolute(x)
...
def calc_Frobenius_Norm(A):
    if not (str(type(A)) == "<class 'numpy.ndarray'>"):
        A = np.array(A)
    A = A.astype(float)
...
print(calc_Condition_Number(A))
print(calc_Condition_Number(B))

s1 = np.linalg.solve(A, np.array([1, 2]))
s2 = np.linalg.solve(B, np.array([1, 2]))
s3 = np.linalg.solve(A, np.array([1.0001, 2.0001]))
s4 = np.linalg.solve(B, np.array([1.0001, 2.0001]))
...
img = Image.open("test_girl.jpg","r")

img_L = img.convert('L')

img_data = np.array(list(img_L.getdata(band=0)))
size = img_L.size
img_data_mat = img_data.reshape(size[1], size[0])

UDVT = calc_svd(img_data_mat)
